...
echo '${RED}[*] Installation de nvim'
mkdir -p ~/.config/nvim

# Install vim-plug plugin manager
echo '${RED}[*] Téléchargement de vim-plug'
...
if [ -f ~/.config/nvim/coc-settings.json ]; then
  echo '${RED}[*] Backup existing coc-settings.json'
...
echo -e '${RED}[*] Running :PlugInstall within nvim ...'
...
echo '${RED}[*] Copying init.vim -> ~/.config/nvim/init.vim'
cp .config/nvim/init.vim ~/.config/nvim/

echo -e "${RED}[+] Installation de neovim terminé."
